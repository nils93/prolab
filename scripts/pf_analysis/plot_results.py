...
def plot_trajectory(pf, gt_interp, rmse_vals, landmarks=None, num_landmarks=None, save_path="export"):
    """
    Zeichnet Trajektorien (PF vs. Ground Truth), Start-/Endpunkte und Landmarken.
    """
    x_p, y_p = pf[:, 1], pf[:, 2]
    ...
    ax.plot(x_p, y_p, 'r-', linewidth=1, label='PF')
    ...
    # (Ellipsen entfernt – PF liefert keine Kovarianzen)
...
def plot_orientation(pf, gt_interp, rmse_vals, save_path="export"):
    """
    Zeichnet Yaw-Winkel (PF vs. Ground Truth) über die Zeit.
    """
    t_rel = pf[:, 0] - pf[0, 0]
    theta_p = pf[:, 3]
    theta_g = gt_interp[:, 3]
    ...
    ax.plot(t_rel, np.degrees(theta_p), 'r-', linewidth=1, label='PF')
...
def draw_covariance_ellipse(ax, x, y, cov, n_std=1.0, **kwargs):
    """
    Zeichnet eine Konfidenzellipse – unverändert.
    """
    if cov.shape != (2, 2):
        return
